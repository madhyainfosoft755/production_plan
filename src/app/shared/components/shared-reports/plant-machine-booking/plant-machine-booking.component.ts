...
@Component({
  ...
  templateUrl: './plant-machine-booking.component.html',
  styleUrl: './plant-machine-booking.component.scss'
})
export class PlantMachineBookingComponent implements OnInit {
  breadcrumbItems: any[];
    ...
    this.adminApiService.plant_machine_booking().subscribe({
      next: (res) => {
        ...
        this.data.forEach(item => {
          const group = grouped.get(item.module_name) || [];
          group.push(item);
          grouped.set(item.module_name, group);
        });

        // 2. Build transformedData with inserted subtotal rows
        this.transformedData = [];

        grouped.forEach((groupItems, moduleName) => {
          // Push group items
          this.transformedData.push(...groupItems);
